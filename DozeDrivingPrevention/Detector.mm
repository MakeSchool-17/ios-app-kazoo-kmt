...
//        NSString *eye1_path = [eye1_bundle pathForResource:@"M. Rezaei- Closed Eye Classifier" ofType:@"xml"];
...
//    NSString *eye2_path = [eyes2_bundle pathForResource:@"M. Rezaei- Closed Eye Classifier" ofType:@"xml"];
...
//    cv::Size(60, 60)); // ここは最終的にスマホの設置位置を決めたうえで調整が必要
    ...
    for(; r1 != face1.end(); ++r1) {
        ...
        r1->y = r1->y + r1->height * 0.2; // 上から20％のところには目は無いとしてカット
        r1->height = r1->height * 0.3; // これでノイズが取れなければ、目の数でスクリーニングする予定.数値は別に宣言すべきか。
        r1->x = r1->x + r1->width * 0.1; // 左から１０％のところには目は無いとしてカット
        r1->width = r1->width * 0.4;
...
//                counterForEye += 1;
    ...
    for(; r2 != face2.end(); ++r2) {
        // Insert value to structure
//        facialFeatures.face.featureRect.origin.x = r2->x;
//        facialFeatures.face.featureRect.origin.y = r2->y;
//        facialFeatures.face.featureRect.size.width = r2->width;
//        facialFeatures.face.featureRect.size.height = r2->height;
//        facialFeatures.face.isDetected = true;
        
        // Cut a lower half part in face
        r2->y = r2->y + r2->height * 0.2; // 上から20％のところには目は無いとしてカット
        r2->height = r2->height * 0.3; // これでノイズが取れなければ、目の数でスクリーニングする予定.数値は別に宣言すべきか。
        r2->x = r2->x + r2->width * 0.5; // 右目のみ特定
        r2->width = r2->width * 0.4;
        // In each face, detect eyes
